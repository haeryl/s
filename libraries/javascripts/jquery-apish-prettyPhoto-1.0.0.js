/* ------------------------------------------------------------------------
	...
	Version	: 3.1.4
	Alter by: Mohd. Hafidz Bin Abdul Kadir
	...
	(function($){
		...
		$.fn.prettyPhoto=function(pp_settings){
			pp_settings=jQuery.extend({
				...
				quicktime_markup:
					'<object classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" codebase="libraries/cabs/qtplugin.cab" height="{height}" width="{width}"> \
				...
				social_tools:
					'<div class="twitter"> \
						...
						<script type="text/javascript" src="libraries/javascripts/jquery-widgets.js"></script> \
					</div> \
					<div class="facebook"> \
					...
					jQuery.map(matchedObjects,function(n,i){
						if($(n).attr(settings.hook).indexOf(theRel)!=-1)
							return($(n).find('img').attr('alt'))?
								$(n).find('img').attr('alt'):"";}):$.makeArray($(this).find('img').attr('alt'));
				...
				$pp_pic_holder.fadeIn(function(){
					...
					switch(_getFileType(pp_images[set_position])){
						...
						case'flash':
							...
							pp_dimensions['width']).replace(/{height}/g,pp_dimensions['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,filename+'?'+flash_vars);
						break;
						case'iframe':
							pp_dimensions=_fitToViewport(movie_width,movie_height);
			...
			$.prettyPhoto.changeGalleryPage=function(direction){
				...
				slide_speed=(direction=='next'||direction=='previous')?settings.animation_speed:0;slide_to=currentGalleryPage*(itemsPerPage*itemWidth);
			...
			};//alter by Hafidz on 16 Januari 2014 (';' sahaja!!!)
